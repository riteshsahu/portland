...
class MessageList extends Component {
  ...
  componentDidMount() {
    ...
    this.props.getRoleChatHistory(JobId);

    window.clientSocket.on('message updated', (result) => {
      console.log("message updated");
      this.props.getRoleChatHistory(this.props.params.id);
    });
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.params.roleKey != this.props.params.roleKey) {
      this.props.getRoleChatHistory(this.props.JobId);
    }
    let subscribe = {
      room: window.location.href.split('/').pop(),
      userId: USER_DETAILS[0].userId
    ...
    // subscribe user to this role chat and unsubscibe him from all other role chats
    if (this.props.roleTab) {
      if (this.props.roleTab !== prevProps.roleTab) {
        window.clientSocket.emit('subscribe', subscribe);
        this.setState({
          messages: []
        })
    ...
    if (this.props.chatHistory.length > 0 && prevProps.chatHistory != this.props.chatHistory) {
      ...
      this.props.chatHistory.forEach((data, index) => {
        ...
        console.log("fromMeBool...", fromMe);

        if (fromMe) {
          // messages sent by me
          if (data.recipientRole == this.props.params.roleKey) {
            // to current role tab
            messages.push({
              // isVisibleToClient: data.isVisibleToClient,
              // room: window.location.href.split('/').pop(),
              // userId: data.createBy,
              author: this.state.KeyRole[data.senderRole] + "-" + data.senderFirstName,
              fromMe: fromMe,
          ...
          // if (fromMe && data.recipientRole == this.props.params.roleKey) {
          //   console.log("fromMe..", data);
            
          //   // message sent by me to current role tab
          //   messages.push({
          //     // isVisibleToClient: data.isVisibleToClient,
          //     // room: window.location.href.split('/').pop(),
          //     // userId: data.createBy,
          //     author: this.state.KeyRole[USER_DETAILS[0].role] + "-" + USER_DETAILS[0].firstName,
          //     fromMe: fromMe,
          //     message: data.message,
          //     fileName: data.fileName,
          //     filePath: data.filePath,
          ...
          //     timestamp: new Date().getTime()
          //   })
          // } else if (!fromMe && data.senderRole == this.props.params.roleKey) {
          //   console.log("toMe..", data);
          //   // message sent to me by current role tab
          //   messages.push({
          //     // isVisibleToClient: data.isVisibleToClient,
          //     // room: window.location.href.split('/').pop(),
          //     // userId: data.createBy,
...
const mapStateToProps = state => {
  return {
    ...
    roleTab: state.RoleChatDetail.roleTab,
...
function mapDispatchToProps(dispatch) {
  return {
    getRoleChatHistory: (id) => dispatch(getRoleChatHistory(id)),
