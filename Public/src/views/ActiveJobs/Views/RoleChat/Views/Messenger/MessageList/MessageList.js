...
class MessageList extends Component {
  ...
  componentDidMount() {
    ...
    this.props.getRoleChatHistory(JobId, this.props.params.roleKey);

    window.clientSocket.on('message updated', (result) => {
      this.props.getRoleChatHistory(this.props.params.id, this.props.params.roleKey);
    });
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.params.roleKey != this.props.params.roleKey) {
      this.props.getRoleChatHistory(this.props.JobId, this.props.params.roleKey);
    ...
    if (this.props.chatHistory.length > 0 && prevProps.chatHistory != this.props.chatHistory) {
      ...
      this.props.chatHistory.forEach((data, index) => {
        ...
        if (fromMe) {
          // messages sent by me
          if (data.recipientRole == this.props.currentRole) {
            // to current role tab
            messages.push({
...
const mapStateToProps = state => {
  return {
    ...
    currentRole: state.RoleChatDetail.role,
...
function mapDispatchToProps(dispatch) {
  return {
    getRoleChatHistory: (id, role) => dispatch(getRoleChatHistory(id, role)),
